...
Noun *UICreation::CreateText(Font* font,
    ...
    auto material = new Material(App::engine_app->output->visual->shader_text);
    material->add_texture(font->texture);

    auto drawable = new Drawable(
            mesh,
            material,
            App::engine_app->output->visual->camera_ui
    );
//    drawable->blend = GL_ONE;
    App::engine_app->output->visual->add_drawable(drawable);
...
Noun *UICreation::CreateImage(Texture *texture, float width, float height) {
    ...
    auto material = new Material(App::engine_app->output->visual->shader_texture_unlit);
